 ...
 *
 * A pump.fun buy/sell instruction always moves tokens between exactly two
 * token accounts for the mint: the trader's ATA and the bonding curve's
 * associated token account. We tell them apart because every parsed token
 * balance entry Solana returns includes an `owner` field: the one whose
 * owner is NOT the transaction's fee-payer/signer is the curve's account.
 */
